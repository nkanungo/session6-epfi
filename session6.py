...
def lambda_func(als : 'list of values'= vals,suits :'List containing suits'= suits):
    '''Lambda function for listing down all the cards '''
    return list(map(lambda x : x[0] +'-'+ x[1] ,zip(vals*len(suits), suits*len(vals))))


def card_combination(vals : 'list of values'= vals,suits :'List containing suits'= suits) -> 'returns deck of 52 cards ':
    ''' This is a simple function which creates a combination that  all the cards in a pack from 2 lists
    '''
    return [ val + '-' + suit for val in vals for suit in suits]

card_conversion_dict = {'ace':14,'king':13,'queen':12,'jack':11,'10':10,'9':9,'8':8,'7':7,'6':6,'5':5,'4':4,'3':3,'2':2}
...
def card_game(player1_card : 'list of cards ' = None , player2_card : 'list of cards ' = None,*args,**kwargs )-> 'returns the winner ':
    ...
    if not bool(player1_card) or not bool(player2_card):
        raise ValueError(' Either You guys are sleeping or one of you wants to run the race alone')
    ...
    player1_card=sorted(set(player1_card),reverse=True)
    player2_card=sorted(set(player2_card),reverse=True)

    if len(player1_card) != len(player2_card):
        raise ValueError(' Please send equal cards for comparision')
    player1_digit_suit_list = [(card_conversion_dict[card.split('-')[0]],card.split('-')[1]) for card in player1_card]
    player1_digit_list, player1_suit_list= zip(*player1_digit_suit_list)

    player2_digit_suit_list = [(card_conversion_dict[card.split('-')[0]],card.split('-')[1]) for card in player2_card]
    player2_digit_list, player2_suit_list= zip(*player2_digit_suit_list)
...
l1 = ['king-hearts', 'king-clubs','9-diamonds', '8-spades','2-hearts']
l2= ['10-hearts', 'jack-hearts','queen-hearts', 'king-hearts','ace-hearts']
l3=['10-hearts', '9-diamonds','7-spades', '3-clubs','2-hearts']
l4=['10-hearts', '9-diamonds','7-spades', '3-clubs','2-clubs']
print(card_game(l3,l4))
